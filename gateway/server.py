    ...
    def dispatch(self, req: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if method == "initialize":
            ...
            init_res["instructions"] = (
                ...
                "4. UNIVERSAL FILE ENGINE & OFFICE WORKFLOWS (NO MOUSE FOR DOCUMENTS):\n"
                "   - STRICT RULE: NEVER use mouse clicks, cursor movements, or screen snapshots to create, format, or edit Word, Excel, text files, or code. The user strictly forbids mouse interference on their screen!\n"
                "   - When the user asks for an Excel table (e.g. 'make a 5x4 table with nice colors') or Word document:\n"
                "     Use `PowerShell` or Python (`openpyxl` / `pandas` / `python-docx`) in the background.\n"
                "     Generate the file with professional formatting (custom header colors, alternating row fills, thin borders, alignments), save it to the Desktop or active folder, and open it cleanly via `Start-Process <file>`.\n"
                "     DO NOT attempt to attach to unsaved `Book1` via `[Marshal]::GetActiveObject` because Windows COM blocks cross-integrity calls (`MK_E_UNAVAILABLE`). Creating and launching the formatted file directly is instantaneous, 100% reliable, and leaves the user's mouse and screen untouched.\n\n"
                ...
                "   - Use `Click`, `Type`, `Shortcut`, or `App` ONLY when interacting with third-party GUI applications that lack any CLI or file interface. NEVER use them for document editing, coding, or terminal operations.\n\n"
